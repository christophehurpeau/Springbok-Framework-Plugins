...
<table class="metas mt10">
	...
	<tr class="{if $auto}auto{else}manuel{/if}">
		...
		<td>
			<? $form->text($k,array('name'=>false,'label'=>false,'id'=>'Seo'.ucFirst($k).'Auto','readonly'=>true,'value'=>$autoResult,'class'=>'wp100 auto'),false) ?>
			<?php $attrs=array('id'=>'Seo'.ucFirst($k),'value'=>$model->$k,'label'=>false,'class'=>'wp100 manuel'.($auto?' autoOnLoad':'')); if($auto) $attrs['disabled']=true; echo $form->text($k,$attrs,false) ?>
		</td>
